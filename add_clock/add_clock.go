...
func init() {
	flag.StringVar(&inputFile, "input", "", "input video file")
	flag.StringVar(&outputFile, "output", "", "output video file")
	flag.StringVar(&textColor, "c", "white", "text color (white or black)")
	flag.StringVar(&timestampType, "t", "datetime", "timestamp format (datetime, date, time)")
	flag.Parse()
}
